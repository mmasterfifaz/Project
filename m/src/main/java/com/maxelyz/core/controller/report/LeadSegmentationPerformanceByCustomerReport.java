...
@ManagedBean
@RequestScoped
public class LeadSegmentationPerformanceByCustomerReport extends AbstractReportBaseController {

    private static Logger log = Logger.getLogger(LeadSegmentationPerformanceByCustomerReport.class);
    ...
    public void initialize() {
        if (!SecurityUtil.isPermitted("report:leadsegmentationperformancebycustomer:view")) {
    ...
    protected String getReportPath() {
        return "/report/LeadSegmentationPerformanceByCustomerReport.jasper";
    }
    ...
    private void generateXLS() {
        ...
        fileName = "Lead_Segmentation_Performance_By_Customer_List_report_" + start + ".xls";
        try {
            file = new FileInputStream(new File(JSFUtil.getuploadPath(), "report" + "/LeadSegmentationPerformanceByCustomer/Lead_Segmentation_Perfromance_By_Customer_report_template.xls"));
            ...
            sheet.getRow(3).getCell(1).setCellValue("Leads Performance By Customer Report");
            ...
            java.util.logging.Logger.getLogger(LeadSegmentationPerformanceByCustomerReport.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            try {
                file.close();
            } catch (Exception ex) {
    ...
    public void retrieveData(HSSFWorkbook workbook, HSSFSheet sheet) throws SQLException {
        ...
        try {
            ...
            CallableStatement callStatement = connection.prepareCall("{call [dbo].[sp_fwd_lead_segmentation_performance_by_customer_header](?,?)}");
            ...
            // create data leads
            callStatement = connection.prepareCall("{call [dbo].[sp_fwd_lead_segmentation_performance_by_customer](?,?)}");
            callStatement.setString(1, sdf1.format(fromDate));
            callStatement.setString(2, sdf1.format(toDate));
            ...
            callStatement = connection.prepareCall("{call [dbo].[sp_fwd_lead_segmentation_performance_by_customer_product](?,?)}");
            callStatement.setString(1, sdf1.format(fromDate));
            callStatement.setString(2, sdf1.format(toDate));
            ...
            //insert Revenue/TSR
            callStatement = connection.prepareCall("{call [dbo].[sp_fwd_lead_segmentation_performance_by_customer_tsr](?,?)}");
            callStatement.setString(1, sdf1.format(fromDate));
            callStatement.setString(2, sdf1.format(toDate));
