...
@RequestScoped
@ViewScoped
public class SlaCaseClosedByContactDateReport extends AbstractReportBaseController {
    private static Logger log = Logger.getLogger(SlaCaseClosedByContactDateReport.class);
    private Integer users_id;
    @PostConstruct
    public void initialize() {
        if (!SecurityUtil.isPermitted("report:slacaseclosedbydate:view")) {
    ...
    public String getReportPath() {
        return "/report/slaCaseClosedByContactDate.jasper";
    }
    ...
    public String getQuery() {
        ...
        String select = "SELECT rpt.contact_date as contact_date, "
                + "sum(rpt.sla_closed1) as '<24', "
                + "sum(rpt.sla_closed2) as '25-48', "
                + "sum(rpt.sla_closed3) as '49-36', "
                + "sum(rpt.sla_closed4) as '>36', "
                + "sum(rpt.sla_closed1+rpt.sla_closed2+rpt.sla_closed3+rpt.sla_closed4) AS total "
        ...
        String groupBy = "GROUP BY rpt.contact_date ";
        String orderBy = "ORDER BY rpt.contact_date ";
