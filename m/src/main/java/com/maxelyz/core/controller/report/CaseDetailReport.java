...
//@RequestScoped
@ViewScoped
public class CaseDetailReport extends AbstractReportBaseController {
    private static Logger log = Logger.getLogger(CaseDetailReport.class);
    ...
    public void initialize() {
        if (!SecurityUtil.isPermitted("report:casedetail:view")) {
    ...
    public String getReportPath() {
        return "/report/caseDetail.jasper";
    }
    ...
    public String getQuery() {
        ...
        String select = "SELECT cases.id, cases.code, cases.contact_date, "
                + "cases.contact_person, c.name as case_type, b.name as case_topic, "
                + "a.name as case_detail, d.name as case_request, cases.contact_person, "
                + "customer.name+' '+customer.surname as customer_name, "
                + "service_type.name as service_type_name, cases.location_name, "
                + "relationship.name relationship_name, channel.type+':'+channel.name as channel_name,"
