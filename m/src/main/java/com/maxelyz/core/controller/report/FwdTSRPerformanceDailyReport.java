...
@ManagedBean
@RequestScoped
public class FwdTSRPerformanceDailyReport {
    private static Logger log = Logger.getLogger(FwdTSRPerformanceDailyReport.class);
    ...
    private void generateXLS() {
        ...
        String fileName = "Daily_Performance_Report_by_TSR_" + date + ".xls";
...
//        trow.createCell(1).setCellValue("0");
    ...
    public void retrieveData(HSSFWorkbook workbook, HSSFSheet sheet) throws SQLException {
        ...
        try {
            ...
            CallableStatement callStatement = connection.prepareCall("{call [dbo].[sp_fwd_sale_report_with_family_by_tsr](?,?,?,?)}");
...
//
            ...
            for(int j = 3 ;j<= totalColumnCountInExcel;j++) {
                ...
                if(j==3){
                    ...
                    hrow4.createCell(j).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                    hrow4.getCell(j).setCellFormula(String.format("IF(%s%d > 0,%s%d/%s%d, 0)",CellReference.convertNumToColString(3),rowTotal+1, CellReference.convertNumToColString(j-3),rowTotal+1, CellReference.convertNumToColString(3),rowTotal+1));
                    hrow4.getCell(j).setCellStyle(headCellStyle2);
                    CellUtil.setCellStyleProperty(hrow4.getCell(j), workbook, CellUtil.DATA_FORMAT, HSSFDataFormat.getBuiltinFormat("0.00%"));
