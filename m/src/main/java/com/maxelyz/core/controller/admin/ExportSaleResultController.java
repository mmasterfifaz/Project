...
@RequestScoped
@ViewScoped
public class ExportSaleResultController implements Serializable{
    private static Logger log = Logger.getLogger(ExportSaleResultController.class);
    ...
    private void generateXLS() {
         ...
         SimpleDateFormat sdfNoTime = new SimpleDateFormat("dd/MM/yyyy", Locale.US);
        ...
        try {
            if (purchaseOrders.size()>0) {
                ...
                for (PurchaseOrder p: purchaseOrders) {
                    ...
                    List<PurchaseOrderRegister> poRegs = (List<PurchaseOrderRegister>)poDetail.getPurchaseOrderRegisterCollection();
                    PurchaseOrderRegister poReg = poRegs.get(0);
                    //QaTransDetail uw = ((List<QaTransDetail>) p.getQaTransApproval().getQaTransDetailCollection()).get(0);
                    ...
                    if(poReg.getDob() != null && p.getPurchaseDate() != null){
                        age = this.calulateAge(poReg.getDob(), p.getPurchaseDate());
                    ...
                    if ("1".equals(poDetail.getProductPlan().getPaymentMode())) {
                        paymentType="Monthly";
                    } else if ("2".equals(poDetail.getProductPlan().getPaymentMode())) {
                        paymentType="Quarterly";
                    } else if ("3".equals(poDetail.getProductPlan().getPaymentMode())) {
                    ...
                    if (poReg.getCurrentSubDistrict()!=null) {
                        writeCol(row, columnNo++, prefixTumbol+poReg.getCurrentSubDistrict().getName(), style); //F
                    } else {
                    ...
                    if (poReg.getCurrentSubDistrict()!=null) {
                        writeCol(row, columnNo++, prefixTumbol+poReg.getCurrentSubDistrict().getName(), style); //R
                    } else {
                        writeCol(row, columnNo++, "", style); // 
                    }
                    if (poReg.getCurrentDistrict()!=null) {
                    ...
                    writeCol(row, columnNo++, poReg.getGender().substring(0,1).equals("M")?"ชาย":"หญิง", style); //Z
                    writeCol(row, columnNo++, maritalStatus, style); //AA
                    if (poReg.getOccupation()!=null) {
                        if (poReg.getOccupation().getGroupName().equals("0"))  {
                            writeCol(row, columnNo++, "Other", style); //AB
                        } else {
                    ...
                    writeCol(row, columnNo++, poDetail.getProductPlan().getCode(), style); //AW
                    ...
                    writeCol(row, columnNo++, dec.format(planDetail != null ? planDetail.getAnnualNetPremium(): 0), style); //BD
                    ...
                    writeCol(row, columnNo++, p.getCardType() != null ? p.getCardType().toUpperCase()+" CARD" : "" , style); //BG
                    writeCol(row, columnNo++, p.getCardHolderName().toUpperCase(), style); //BH

                    String cardExpiryMonth = "";
                    if(p.getCardExpiryMonth() != null){
                    ...
                    //writeCol(row, columnNo++, uw.getAnswerDesc(), style); //BJ
                    //writeCol(row, columnNo++, uw.getRemark(), style); //BK
                    writeCol(row, columnNo++, "", style); //BJ
                    writeCol(row, columnNo++, "", style); //BK
                    ...
                    writeCol(row, columnNo++, poReg.getOccupationText(), style); //BN  Remark
    ...
    private void generatePendingPaymentXLS(boolean approvalStatus) {
      List<PurchaseOrder> purchaseOrders = this.getPurchaseOrderDAO().findPurchaseOrderBySaleApproval(fromDate, toDate, approvalStatus);
         SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy HH:mm", Locale.US);
         SimpleDateFormat sdfNoTime = new SimpleDateFormat("dd/MM/yyyy", Locale.US);
         SimpleDateFormat sdfFileName = new SimpleDateFormat("yyyyMMdd", Locale.US);
        ...
        FacesContext context = FacesContext.getCurrentInstance();
        HttpServletResponse response = (HttpServletResponse) context.getExternalContext().getResponse();
        response.setHeader("Content-Disposition", "Attachment;filename=pengding_payement"+sdfFileName.format(new Date())+".xls");
        OutputStream out = null;
        ...
        try {
            if (purchaseOrders.size()>0) {
                ...
                row = sheet1.createRow(rowNo++);
                writeCol(row,columnNo++,"Reference_No", style); //A
                writeCol(row,columnNo++,"Insured_Initial", style); //B
                writeCol(row,columnNo++,"Insured_FirstName", style); //C
                writeCol(row,columnNo++,"Insured_LastName", style); //D
                writeCol(row,columnNo++,"Insured_Telephone1", style); //E
                writeCol(row,columnNo++,"Insured_Telephone2", style); //F
                writeCol(row,columnNo++,"Insured_Telephone3", style); //G
                
                writeCol(row,columnNo++,"Policy_Product", style); //H
                writeCol(row,columnNo++,"Policy_Plan", style); //H
                writeCol(row,columnNo++,"Policy_NetPremium", style); //I
                writeCol(row,columnNo++,"Payment_Amount", style); //J
                writeCol(row,columnNo++,"Payment_Type", style); //K
                writeCol(row,columnNo++,"Purchased_Date", style); //L
                writeCol(row,columnNo++,"Credit_IDCard", style); //M
                writeCol(row,columnNo++,"Credit_Type", style); //N
                writeCol(row,columnNo++,"Credit_CardName", style); //O
                writeCol(row,columnNo++,"CreditCardExpDate", style); //P
                
                writeCol(row,columnNo++,"Payer_Initial", style); //Q
                writeCol(row,columnNo++,"Payer_FirstName", style); //R
                writeCol(row,columnNo++,"Payer_LastName", style); //S
                writeCol(row,columnNo++,"Payer_Relation", style); //T
                writeCol(row,columnNo++,"Payer_Telephone1", style); //E
                writeCol(row,columnNo++,"Payer_Telephone2", style); //F
                writeCol(row,columnNo++,"Payer_Telephone3", style); //G
                
                writeCol(row,columnNo++,"AgentID", style); //U
                writeCol(row,columnNo++,"t_tsrName", style); //V
                ...
                for (PurchaseOrder p: purchaseOrders) {
                    ...
                    if ("1".equals(poDetail.getPaymentMode())) {
                        paymentType="Monthly";
                    } else if ("2".equals(poDetail.getPaymentMode())) {
                        paymentType="Quarterly";
                    } else if ("3".equals(poDetail.getPaymentMode())) {
                        paymentType="Semi-annual";
                    } else if ("4".equals(poDetail.getPaymentMode())) {
                        paymentType="Yearly";
                    }
                    //Marital Status
                    String maritalStatus="";
                    ...
                    columnNo=0;
                    writeCol(row, columnNo++, p.getRefNo(), style); //A
                    writeCol(row, columnNo++, poReg.getInitialLabel(), style); //B
                    writeCol(row, columnNo++, poReg.getName(), style); //C
                    writeCol(row, columnNo++, poReg.getSurname(), style); //D
                    writeCol(row, columnNo++, poReg.getMobilePhone(), style); //E
                    writeCol(row, columnNo++, poReg.getHomePhone(), style); //F
                    writeCol(row, columnNo++, poReg.getOfficePhone(), style); //LG
                    
                    writeCol(row, columnNo++, poDetail.getProduct().getCode(), style); //H
                    writeCol(row, columnNo++, poDetail.getProductPlan().getCode(), style); //H
                    writeCol(row, columnNo++, dec.format(poDetail.getUnitPrice() != null ? poDetail.getUnitPrice() : 0), style);//I
                   
                    if ("1".equals(poDetail.getPaymentMode())) {
                        System.out.println("No Payment : " + poDetail.getProduct().getMonthlyFirstPayment());
                        writeCol(row, columnNo++, dec.format(poDetail != null ? poDetail.getUnitPrice() * poDetail.getProduct().getMonthlyFirstPayment() : 0), style); //BB
                    } else if ("2".equals(poDetail.getPaymentMode())) {
                        writeCol(row, columnNo++, dec.format(poDetail != null ? poDetail.getUnitPrice() * poDetail.getProduct().getQuarterlyFirstPayment(): 0), style); //BB
                    } else {
                        writeCol(row, columnNo++, dec.format(poDetail != null ? poDetail.getUnitPrice(): 0), style); //BB
                    }//J
                    writeCol(row, columnNo++, paymentType, style); //K
                    writeCol(row, columnNo++, p.getPurchaseDate()!= null ? sdfNoTime.format(p.getPurchaseDate()) : "", style); //L
                    writeCol(row, columnNo++, p.getCardNo(), style); //M
                    writeCol(row, columnNo++, p.getCardType() != null ? p.getCardType().toUpperCase()+" CARD" : "" , style); //N
                    writeCol(row, columnNo++, p.getCardHolderName().toUpperCase(), style); //O
                    ...
                    writeCol(row, columnNo++, cardExpiryMonth+"/"+p.getCardExpiryYear(), style); //P
                    
                    writeCol(row, columnNo++, p.getPayerInitialText(), style); //Q
                    writeCol(row, columnNo++, p.getPayerName(), style); //R
                    writeCol(row, columnNo++, p.getPayerSurname(), style); //S
                    writeCol(row, columnNo++, p.getPayerRelation() != null ? p.getPayerRelation() : "ตนเอง", style); //T
                    writeCol(row, columnNo++, p.getPayerMobilePhone(), style); //T
                    writeCol(row, columnNo++, p.getPayerOfficePhone(), style); //T
                    writeCol(row, columnNo++, p.getPayerHomePhone(), style); //T
                    
                    writeCol(row, columnNo++, p.getCreateByUser().getId().toString(), style); //U
                    writeCol(row, columnNo++, p.getCreateByUser().getName()+" "+p.getCreateByUser().getSurname(), style); //V
    ...
    public void searchActionListener(ActionEvent event) {       
        saleApprovalValues = this.purchaseOrderDAO.findPurchaseOrderBySaleApproval(fromDate, toDate, "");
    }
    ...
    public void exportPendingListener(ActionEvent event) {
        generatePendingPaymentXLS(false);
        FacesContext.getCurrentInstance().responseComplete();
    }
