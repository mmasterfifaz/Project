...
@ManagedBean
@RequestScoped
public class AttendanceReport extends AbstractReportBaseController {
    private static Logger log = Logger.getLogger(SaleByRecordsReport.class);
    @PostConstruct
    public void initialize() {
        if (!SecurityUtil.isPermitted("report:attendancereport:view")) {
    ...
    public String getReportPath() {
        return "/report/attendanceReport.jasper";
    }
    ...
    public String getQuery() {
        String select = "{call GetAttendanceData('"+ sdf.format(fromDate) + "','" + sdf.format(toDate) + "'," + userGroupId + "," + userId + ")}";
