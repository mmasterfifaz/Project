...
//@RequestScoped
@ViewScoped
public class ImportPaymentController {
    private static Logger log = Logger.getLogger(ImportPaymentController.class);
...
//    private UploadItem item;
    ...
    private Integer ref1Col,ref2Col,amountCol,dateCol,idCardCol;
    ...
    public void initialize() {
        if (!SecurityUtil.isPermitted("admin:importpayment:view")) {
...
//    }  
    ...
    public void uploadListener(FileUploadEvent event) throws Exception{
     ...
     public void readExcelFile(UploadedFile item)   {   
        try {
            fileInputStream =  new FileInputStream(item.getName());//item.getInputStream();// new FileInputStream(item.getFile().getPath());
                ...
                if(item.getName().contains(".xlsx")) {
                    ...
                    if(rowIterator.hasNext()) {     //check header row
                        ...
                        while(iterator.hasNext())  {   //find column index --> REF1 REF2 AMOUNT DATE
                            ...
                            if(xssfCell.getStringCellValue().toString().equals("REF1")) {  //poRefNo or IdCard
                                ref1Col = xssfCell.getColumnIndex();
                            } else if(xssfCell.getStringCellValue().toString().equals("REF2")) {   //ID Card
                                        ref2Col = xssfCell.getColumnIndex();
                            } else if(xssfCell.getStringCellValue().toString().equals("AMOUNT")) {
                                        amountCol = xssfCell.getColumnIndex();
                            } else if(xssfCell.getStringCellValue().toString().equals("DATE")) {
                    ...
                    while(rowIterator.hasNext()) {  //check Po.Ref_no, po.customer_ref_no, po.purchase_date, po.amount  --> save settlement detail
                        ...
                        xssfCell = xssfRow.getCell(ref1Col, Row.CREATE_NULL_AS_BLANK);
                        xssfCell.setCellType(Cell.CELL_TYPE_STRING);
                        String refNo = xssfCell.getStringCellValue().toString();

                        xssfCell = xssfRow.getCell(ref2Col, Row.CREATE_NULL_AS_BLANK);
                        xssfCell.setCellType(Cell.CELL_TYPE_STRING);
                        String customerRefno = xssfCell.getStringCellValue().toString();
...
//                        xssfCell = xssfRow.getCell(idCardCol, Row.CREATE_NULL_AS_BLANK);
//                        xssfCell.setCellType(Cell.CELL_TYPE_STRING);
//                        String idCard = xssfCell.getStringCellValue().toString();
                        
                        SettlementDetail settlementDetail = new SettlementDetail();
                        PurchaseOrderInstallment poi = getPurchaseOrderDAO().findPurchaseOrderInstallmentSettlement(refNo, amount);
                        String result="";
                        if(poi != null){    // match
                        ...
                        settlementDetail.setCustomerRefNo(customerRefno);
                    ...
                    if(rowIterator.hasNext()) {     //check header row
                        ...
                        while(iterator.hasNext())  {   //find column index --> REF1 REF2 AMOUNT DATE ID_CARD
                            HSSFCell hssfCell = (HSSFCell) iterator.next(); 
                            if(hssfCell.getStringCellValue().toString().equals("REF1")) {
                                ref1Col = hssfCell.getColumnIndex();
                            } else if(hssfCell.getStringCellValue().toString().equals("REF2")) {
                                        ref2Col = hssfCell.getColumnIndex();
                            } else if(hssfCell.getStringCellValue().toString().equals("AMOUNT")) {
                                        amountCol = hssfCell.getColumnIndex();
                            } else if(hssfCell.getStringCellValue().toString().equals("DATE")) {
                    ...
                    while(rowIterator.hasNext()) {  //check Po.Ref_no, po.customer_ref_no, po.purchase_date, po.amount  --> save settlement detail
                        ...
                        hssfCell = hssfRow.getCell(ref1Col, Row.CREATE_NULL_AS_BLANK);
                        hssfCell.setCellType(Cell.CELL_TYPE_STRING);
                        String refNo = hssfCell.getStringCellValue().toString();

                        hssfCell = hssfRow.getCell(ref2Col, Row.CREATE_NULL_AS_BLANK);
                        hssfCell.setCellType(Cell.CELL_TYPE_STRING);
                        String customerRefno = hssfCell.getStringCellValue().toString();
...
//                        hssfCell = hssfRow.getCell(idCardCol, Row.CREATE_NULL_AS_BLANK);
//                        hssfCell.setCellType(Cell.CELL_TYPE_STRING);
//                        String idCard = hssfCell.getStringCellValue().toString();
    ...
    public Date convertDate(String dateStr){
        ...
        String newDate= "";  //dd=0, MM=1 yyyy=2
        
        Integer tempYear = Integer.valueOf(dateSplit[2])-543;     //change year th to en -543
        newDate = tempYear.toString()+"-"+dateSplit[1]+"-"+dateSplit[0];
        
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd", Locale.US);
        Date dateTemp= new Date();
        try {
            dateTemp = dateFormat.parse(newDate);
        }catch (Exception e) {
            log.error(e);
        }
        return dateTemp;
