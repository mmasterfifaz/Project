...
@ManagedBean
@RequestScoped
public class FwdAgentPerformanceDailyReport {
    private static Logger log = Logger.getLogger(FwdAgentPerformanceDailyReport.class);
    ...
    private void generateXLS() {
        ...
        String fileName = "Daily_Sales_by_Agent_" + date + ".xls";
...
//        trow.createCell(1).setCellValue("0");
        ...
        hrow1.createCell(colnum).setCellValue("Supervisor"); hrow1.getCell(colnum).setCellStyle(cellStyle);
        ...
        hrow1.createCell(colnum).setCellValue("Marketing"); hrow1.getCell(colnum).setCellStyle(cellStyle);
        hrow2.createCell(colnum).setCellValue(""); hrow2.getCell(colnum).setCellStyle(cellStyle);
        hrow3.createCell(colnum).setCellValue(""); hrow3.getCell(colnum++).setCellStyle(cellAltStyle);
        sheet.addMergedRegion(CellRangeAddress.valueOf(String.format("%s%d:%s%d", CellReference.convertNumToColString(colnum - 1), rownum + 1, CellReference.convertNumToColString(colnum - 1), rownum + 3)));
        ...
        hrow1.createCell(colnum).setCellValue("Contactable"); hrow1.getCell(colnum).setCellStyle(cellStyle);
        hrow2.createCell(colnum).setCellValue(""); hrow2.getCell(colnum).setCellStyle(cellStyle);
        hrow3.createCell(colnum).setCellValue(""); hrow3.getCell(colnum++).setCellStyle(cellStyle);
        sheet.addMergedRegion(CellRangeAddress.valueOf(String.format("%s%d:%s%d", CellReference.convertNumToColString(colnum - 1), rownum + 1, CellReference.convertNumToColString(colnum - 1), rownum + 3)));
    ...
    public void retrieveData(HSSFWorkbook workbook, HSSFSheet sheet) throws SQLException {
        ...
        try {
            ...
            CallableStatement callStatement = connection.prepareCall("{call [dbo].[sp_fwd_sale_report_with_family_by_agent](?,?,?,?)}");
            ...
            int startRowIndx = 12;
            int startRowTotalOFTSR = startRowIndx;
            ...
            LinkedHashMap<String,Integer> totalSubmitAFYPEachOfProductByTSR =new LinkedHashMap<String,Integer>();
            LinkedHashMap<String,Integer> totalSuccessAFYPEachOfProductByTSR =new LinkedHashMap<String,Integer>();

            LinkedHashMap<String,Integer> totalSuccessAPPEachOfProductByTSR =new LinkedHashMap<String,Integer>();

            boolean isLatestTotalOfTsr = false;
            ...
            if (rs.next()) {
               do {
                   ...
                   while( i<= colCount) {
                   // part of 1
                       if(sheet.getRow(rowIndx)==null) { // create new row after calculate total
                           dataRow = sheet.createRow(rowIndx);
                       }
                       ...
                       if (i == 1) {
                           ...
                           sheet.getRow(rowIndx).getCell(colIndx).setCellStyle(textCellStyle);
                           supervisor = rs.getString(i);
                           colIndx++;
                       }
                       ...
                       if (i == 2) {
                           ...
                           sheet.getRow(rowIndx).getCell(colIndx).setCellStyle(textCellStyle);
                           if(rowIndx>12){
                               if(!TSR.equals(rs.getString(i))){
                                   // set total each of TSR
                                   sheet.getRow(rowIndx).getCell(colIndx-1).setCellStyle(textCellStyle);
                                   sheet.getRow(rowIndx).getCell(colIndx).setCellStyle(headCellStyle);
                                   sheet.getRow(rowIndx).getCell(colIndx).setCellValue("Total " + TSR);
                                   TSR = rs.getString(i);

                                   sheet.addMergedRegion(CellRangeAddress.valueOf(String.format("%s%d:%s%d", CellReference.convertNumToColString(1), rowIndx+1, CellReference.convertNumToColString(2),rowIndx + 1)));
                                   calculateTotalOfTSR(workbook,sheet,hrow1,hrow2,hrow3,rowIndx,totalColumnCountInExcel,3,startRowTotalOFTSR,totalFollowEachOfTSR,totalSuccessAFYPEachOfProductByTSR,totalSubmitAFYPEachOfProductByTSR,totalSuccessAPPEachOfProductByTSR);

                                   // clear for calculate new TSR
                                   totalSubmitAFYPEachOfProductByTSR.clear();
                                   totalSuccessAFYPEachOfProductByTSR.clear();
                                   totalSuccessAPPEachOfProductByTSR.clear();

                                   startRowTotalOFTSR = rowIndx+1;
                                   rowIndx++;
                                   colIndx = 0;
                       ...
                       if (i == 6) {
                           //set total follow distinct each of TSR
                           totalFollowEachOfTSR = rs.getInt(i);
                       }
                       if (i == 7) {
                           //set total follow distinct
                           totalFollow = rs.getInt(i);
                       }
                       if (i == 8) {
                           //set total
                           sheet.getRow(rowIndx).createCell(colIndx).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                       ...
                       if (i == 10) {
                           //Call Attempt Rate
                           sheet.getRow(rowIndx).createCell(colIndx).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                           sheet.getRow(rowIndx).getCell(colIndx).setCellFormula(String.format("IF(%s%d > 0,%s%d/%s%d, 0)", CellReference.convertNumToColString(5), rowIndx +1,CellReference.convertNumToColString(6), rowIndx + 1, CellReference.convertNumToColString(5), rowIndx +1));
                           sheet.getRow(rowIndx).getCell(colIndx).setCellStyle(numberCellStyle);
                       ...
                       if (i == 12) {
                           //set Contactable %
                           sheet.getRow(rowIndx).createCell(colIndx).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                           sheet.getRow(rowIndx).getCell(colIndx).setCellFormula(String.format("IF(%s%d > 0,%s%d/%s%d, 0)", CellReference.convertNumToColString(5), rowIndx + 1, CellReference.convertNumToColString(8), rowIndx + 1, CellReference.convertNumToColString(5), rowIndx + 1));
                           sheet.getRow(rowIndx).getCell(colIndx).setCellStyle(numberCellStyle);
                           CellUtil.setCellStyleProperty(sheet.getRow(rowIndx).getCell(colIndx), workbook, CellUtil.DATA_FORMAT, HSSFDataFormat.getBuiltinFormat("0.00%"));
                           colIndx++;
                       }
                       ...
                       if (i == 14) {
                           ...
                           sheet.getRow(rowIndx).createCell(colIndx).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                           sheet.getRow(rowIndx).getCell(colIndx).setCellFormula(String.format("IF(%s%d > 0,%s%d/%s%d, 0)", CellReference.convertNumToColString(8), rowIndx + 1,  CellReference.convertNumToColString(10),rowIndx + 1, CellReference.convertNumToColString(8), rowIndx + 1));
                           sheet.getRow(rowIndx).getCell(colIndx).setCellStyle(numberCellStyle);
                           CellUtil.setCellStyleProperty(sheet.getRow(rowIndx).getCell(colIndx), workbook, CellUtil.DATA_FORMAT, HSSFDataFormat.getBuiltinFormat("0.00%"));
                           lastRow = rowIndx;
                           lastCols = colIndx;
                       ...
                       if(isSubmit) {
                           ...
                           if (isSubmitAFYPNoFamily) {
                               ...
                               if(rowIndx==12) {totalSubmitAFYPEachOfProduct.put(rs.getMetaData().getColumnName(i),rs.getInt(i));
                                   totalSubmitAFYPEachOfProductByTSR.put(rs.getMetaData().getColumnName(i), rs.getInt(i));
                               }

                               if(totalSubmitAFYPEachOfProductByTSR.get(rs.getMetaData().getColumnName(i))==null) {
                                   totalSubmitAFYPEachOfProductByTSR.put(rs.getMetaData().getColumnName(i), total2);
                               }
                               ...
                               if(rowIndx>12) {
                                   ...
                                   int oldDataSubmitByTSR = totalSubmitAFYPEachOfProductByTSR.get(rs.getMetaData().getColumnName(i));
                                   total2 = oldDataSubmitByTSR + rs.getInt(i);
                                   totalSubmitAFYPEachOfProductByTSR.put(rs.getMetaData().getColumnName(i), total2);
                               }
                       ...
                       if(isSuccess) {
                           ...
                           if (isSuccessAppsNoFamily) {
                               ...
                               if(rowIndx==12) {totalSuccessAPPEachOfProduct.put(rs.getMetaData().getColumnName(i),rs.getInt(i));
                                   totalSuccessAPPEachOfProductByTSR.put(rs.getMetaData().getColumnName(i), rs.getInt(i));
                               }

                               if(totalSuccessAPPEachOfProductByTSR.get(rs.getMetaData().getColumnName(i))==null){
                                   totalSuccessAPPEachOfProductByTSR.put(rs.getMetaData().getColumnName(i), total2);
                               }
                               ...
                               if(rowIndx>12) {
                                   ...
                                   int oldDataSuccessByTSR = totalSuccessAPPEachOfProductByTSR.get(rs.getMetaData().getColumnName(i));
                                   total2 = oldDataSuccessByTSR + rs.getInt(i);
                                   totalSuccessAPPEachOfProductByTSR.put(rs.getMetaData().getColumnName(i), total2);
                               }
                           ...
                           if (isSuccessAFYPNoFamily) {
                               ...
                               if(rowIndx==12) {totalSuccessAFYPEachOfProduct.put(rs.getMetaData().getColumnName(i),rs.getInt(i));
                                   totalSuccessAFYPEachOfProductByTSR.put(rs.getMetaData().getColumnName(i),rs.getInt(i));
                               }

                               if(totalSuccessAFYPEachOfProductByTSR.get(rs.getMetaData().getColumnName(i))==null){
                                   totalSuccessAFYPEachOfProductByTSR.put(rs.getMetaData().getColumnName(i),total2);
                               }
                               ...
                               if(rowIndx>12) {
                                   ...
                                   int oldDataSuccessByTSR = totalSuccessAFYPEachOfProductByTSR.get(rs.getMetaData().getColumnName(i));
                                   total2 = oldDataSuccessByTSR + rs.getInt(i);
                                   totalSuccessAFYPEachOfProductByTSR.put(rs.getMetaData().getColumnName(i), total2);
                               }
...
//
                               ...
                               sheet.getRow(rowIndx).getCell(startNetPremiumRejectWithFamily + 1).setCellFormula(String.format("IF(%s%d > 0,%s%d/(%d+%s%d), 0)", CellReference.convertNumToColString(startNetPremiumRejectNoFamily-1), lastRow + 1, CellReference.convertNumToColString(startNetPremiumRejectNoFamily-1), lastRow + 1, successApp,CellReference.convertNumToColString(startNetPremiumRejectNoFamily-1), lastRow + 1));
                ...
                isLatestTotalOfTsr = true;
            }

            if(isLatestTotalOfTsr) {
                lastRow++;
                dataRow = sheet.createRow(rowIndx);
                sheet.getRow(rowIndx).createCell(0).setCellType(HSSFCell.CELL_TYPE_STRING);
                sheet.getRow(rowIndx).getCell(0).setCellStyle(textCellStyle);
                sheet.getRow(rowIndx).getCell(0).setCellValue(supervisor);
                sheet.getRow(rowIndx).createCell(1).setCellType(HSSFCell.CELL_TYPE_STRING);
                sheet.getRow(rowIndx).getCell(1).setCellStyle(headCellStyle);
                sheet.getRow(rowIndx).getCell(1).setCellValue("Total " + TSR);
                sheet.getRow(rowIndx).createCell(2).setCellType(HSSFCell.CELL_TYPE_STRING);
                sheet.getRow(rowIndx).getCell(2).setCellStyle(headCellStyle);

                sheet.addMergedRegion(CellRangeAddress.valueOf(String.format("%s%d:%s%d", CellReference.convertNumToColString(1), lastRow + 1, CellReference.convertNumToColString(2), lastRow + 1)));
                calculateTotalOfTSR(workbook, sheet, hrow1, hrow2, hrow3, rowIndx, totalColumnCountInExcel, 3, startRowTotalOFTSR, totalFollowEachOfTSR, totalSuccessAFYPEachOfProductByTSR, totalSubmitAFYPEachOfProductByTSR, totalSuccessAPPEachOfProductByTSR);
                ...
                hrow4.createCell(1).setCellValue("Total");
                hrow4.getCell(1).setCellStyle(headCellStyle);
                hrow4.createCell(1).setCellValue("Total");
                hrow4.getCell(1).setCellStyle(headCellStyle);
                hrow4.createCell(2).setCellValue("Total");
                hrow4.getCell(2).setCellStyle(headCellStyle);
                ...
                for(int j = 3 ;j<= totalColumnCountInExcel;j++) {
                    String rowAndColumn[] = rowEachOfTotal.split(",");
                    String formula = "";
                    for(String c : rowAndColumn){
                        if(formula.equals(""))
                            formula = CellReference.convertNumToColString(j)+""+c;
                        else
                            formula += ","+CellReference.convertNumToColString(j)+""+c;
                    }
                    ...
                    if(j==3){
                        ...
                        hrow4.getCell(j).setCellFormula(String.format("SUM(" + formula + ")", CellReference.convertNumToColString(j), startData, CellReference.convertNumToColString(j), lastData));
                        ...
                        // set Contactable %
                        hrow4.createCell(j).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                        hrow4.getCell(j).setCellFormula(String.format("IF(%s%d > 0,%s%d/%s%d, 0)", CellReference.convertNumToColString(j - 4), rowTotal + 1, CellReference.convertNumToColString(j - 1), rowTotal + 1, CellReference.convertNumToColString(j - 4), rowTotal + 1));
                        hrow4.getCell(j).setCellStyle(headCellStyle2);
                        CellUtil.setCellStyleProperty(hrow4.getCell(j), workbook, CellUtil.DATA_FORMAT, HSSFDataFormat.getBuiltinFormat("0.00%"));
                    } else if(j==11){
                        // set Sales Close Rate %
                        ...
                        hrow4.createCell(j).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                        hrow4.getCell(j).setCellFormula(String.format("ROUND(IF(%s%d > 0,%s%d/%s%d, 0),2)", CellReference.convertNumToColString(3), rowTotal + 1, CellReference.convertNumToColString(j - 3), rowTotal + 1, CellReference.convertNumToColString(3), rowTotal + 1));
                        hrow4.getCell(j).setCellStyle(headCellStyle2);
                        CellUtil.setCellStyleProperty(hrow4.getCell(j), workbook, CellUtil.DATA_FORMAT, HSSFDataFormat.getBuiltinFormat("0.00%"));
    ...
    public void calculateTotalOfTSR(HSSFWorkbook workbook,HSSFSheet sheet,HSSFRow hrow1,HSSFRow hrow2,HSSFRow hrow3,Integer rowIndx,Integer totalColumnCountInExcel,Integer startColumn,Integer startRowTotalOFTSR,Integer totalFollowEachOfTSR,Map<String,Integer> totalSuccessAFYPEachOfProductByTSR,Map<String,Integer> totalSubmitAFYPEachOfProductByTSR,Map<String,Integer> totalSuccessAPPEachOfProductByTSR){

        if(rowEachOfTotal.equals("")){
            rowEachOfTotal = (rowIndx+1)+"";
        }else{
            rowEachOfTotal += ","+(rowIndx+1);
        }
        ...
        int totalSuccessAFYPEachOfTSR =0;
        int totalSubmitAFYPEachOfTSR = 0;

        // code to get the style for the cell goes here
        headCellStyle2.setFillForegroundColor(palIndex);
        headCellStyle2.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);

        sheet.getRow(rowIndx).getCell(0).setCellValue(sheet.getRow(rowIndx-1).getCell(0).getStringCellValue());

        for(int j = startColumn ;j<= totalColumnCountInExcel;j++) {

            if(j==3){
                sheet.getRow(rowIndx).createCell(j).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                sheet.getRow(rowIndx).getCell(j).setCellFormula(String.format("SUM(%s%d:%s%d)", CellReference.convertNumToColString(j), startRowTotalOFTSR+1, CellReference.convertNumToColString(j), rowIndx));
                sheet.getRow(rowIndx).getCell(j).setCellStyle(headCellStyle2);
                CellUtil.setCellStyleProperty(sheet.getRow(rowIndx).getCell(j), workbook, CellUtil.DATA_FORMAT, HSSFDataFormat.getBuiltinFormat("#,##0"));
            } else if(j==4){
                //set total follow each of tsr
                sheet.getRow(rowIndx).createCell(j).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                sheet.getRow(rowIndx).getCell(j).setCellValue(totalFollowEachOfTSR);
                sheet.getRow(rowIndx).getCell(j).setCellStyle(headCellStyle2);
                CellUtil.setCellStyleProperty(sheet.getRow(rowIndx).getCell(j), workbook, CellUtil.DATA_FORMAT, HSSFDataFormat.getBuiltinFormat("#,##0"));
            } else if(j==5){
                //set total follow (distinct)+ total new
                sheet.getRow(rowIndx).createCell(j).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                sheet.getRow(rowIndx).getCell(j).setCellFormula(String.format("(%s%d+%s%d)", CellReference.convertNumToColString(j-2), rowIndx+1 , CellReference.convertNumToColString(j-1), rowIndx+1));
                sheet.getRow(rowIndx).getCell(j).setCellStyle(headCellStyle2);
                CellUtil.setCellStyleProperty( sheet.getRow(rowIndx).getCell(j), workbook, CellUtil.DATA_FORMAT, HSSFDataFormat.getBuiltinFormat("#,##0"));
            } else if(j==7){
                // set Total Call Attempt Rate
                sheet.getRow(rowIndx).createCell(j).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                sheet.getRow(rowIndx).getCell(j).setCellFormula(String.format("IF(%s%d > 0,%s%d/%s%d, 0)", CellReference.convertNumToColString(j-2), rowIndx+1, CellReference.convertNumToColString(j-1), rowIndx+1,CellReference.convertNumToColString(j-2), rowIndx+1));
                sheet.getRow(rowIndx).getCell(j).setCellStyle(headCellStyle2);
                CellUtil.setCellStyleProperty(sheet.getRow(rowIndx).getCell(j), workbook, CellUtil.DATA_FORMAT, HSSFDataFormat.getBuiltinFormat("#,##0.00"));
            } else if(j==9){
                // set Contactable %
                sheet.getRow(rowIndx).createCell(j).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                sheet.getRow(rowIndx).getCell(j).setCellFormula(String.format("IF(%s%d > 0,%s%d/%s%d, 0)", CellReference.convertNumToColString(j - 4), rowIndx + 1, CellReference.convertNumToColString(j - 1), rowIndx + 1, CellReference.convertNumToColString(j - 4), rowIndx + 1));
                sheet.getRow(rowIndx).getCell(j).setCellStyle(headCellStyle2);
                CellUtil.setCellStyleProperty(sheet.getRow(rowIndx).getCell(j), workbook, CellUtil.DATA_FORMAT, HSSFDataFormat.getBuiltinFormat("0.00%"));
            } else if(j==11){
                // set Sales Close Rate %
                sheet.getRow(rowIndx).createCell(j).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                sheet.getRow(rowIndx).getCell(j).setCellFormula(String.format("IF(%s%d > 0,%s%d/%s%d, 0)", CellReference.convertNumToColString(j - 3), rowIndx + 1, CellReference.convertNumToColString(j - 1), rowIndx + 1, CellReference.convertNumToColString(j - 3), rowIndx + 1));
                sheet.getRow(rowIndx).getCell(j).setCellStyle(headCellStyle2);
                CellUtil.setCellStyleProperty(sheet.getRow(rowIndx).getCell(j), workbook, CellUtil.DATA_FORMAT, HSSFDataFormat.getBuiltinFormat("0.00%"));
            } else if(hrow3.getCell(j).getStringCellValue().trim().equals("AARP")){
                sheet.getRow(rowIndx).createCell(j).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                ...
                sheet.getRow(rowIndx).createCell(j).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                sheet.getRow(rowIndx).getCell(j).setCellFormula(String.format("IF(%s%d > 0,%s%d/%s%d, 0)", CellReference.convertNumToColString(j-3), rowIndx+1, CellReference.convertNumToColString(j-2), rowIndx+1,CellReference.convertNumToColString(j-3), rowIndx+1));
                sheet.getRow(rowIndx).getCell(j).setCellStyle(headCellStyle2);
                CellUtil.setCellStyleProperty(sheet.getRow(rowIndx).getCell(j), workbook, CellUtil.DATA_FORMAT, HSSFDataFormat.getBuiltinFormat("#,##0"));
            } else if(hrow1.getCell(j).getStringCellValue().trim().equals("List Conversion %")){
                sheet.getRow(rowIndx).createCell(j).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                sheet.getRow(rowIndx).getCell(j).setCellFormula(String.format("ROUND(IF(%s%d > 0,%s%d/%s%d, 0),2)",CellReference.convertNumToColString(3),rowIndx+1, CellReference.convertNumToColString(j-3),rowIndx+1, CellReference.convertNumToColString(3),rowIndx+1));
                sheet.getRow(rowIndx).getCell(j).setCellStyle(headCellStyle2);
                CellUtil.setCellStyleProperty(sheet.getRow(rowIndx).getCell(j), workbook, CellUtil.DATA_FORMAT, HSSFDataFormat.getBuiltinFormat("0.00%"));
            } else if(hrow1.getCell(j).getStringCellValue().trim().equals("Conversion Rate %")){
                if(hrow2.getCell(j).getStringCellValue().trim().equals("Total")){
                    sheet.getRow(rowIndx).createCell(j).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                    sheet.getRow(rowIndx).getCell(j).setCellFormula(String.format("IF(%d > 0,%d/%d, 0)",totalSubmitAFYPEachOfTSR,totalSuccessAFYPEachOfTSR,totalSubmitAFYPEachOfTSR));
                    sheet.getRow(rowIndx).getCell(j).setCellStyle(headCellStyle2);
                    CellUtil.setCellStyleProperty(sheet.getRow(rowIndx).getCell(j), workbook, CellUtil.DATA_FORMAT, HSSFDataFormat.getBuiltinFormat("0.00%"));
                }else {
                    int submitAFYP = totalSubmitAFYPEachOfProductByTSR.get(hrow2.getCell(j).getStringCellValue().trim());
                    totalSubmitAFYPEachOfTSR += submitAFYP;
                    int successAFYP = totalSuccessAFYPEachOfProductByTSR.get(hrow2.getCell(j).getStringCellValue().trim());
                    totalSuccessAFYPEachOfTSR += successAFYP;
                    sheet.getRow(rowIndx).createCell(j).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                    sheet.getRow(rowIndx).getCell(j).setCellFormula(String.format("IF(%d > 0,%d/%d, 0)",submitAFYP,successAFYP,submitAFYP));
                    sheet.getRow(rowIndx).getCell(j).setCellStyle(headCellStyle2);
                    CellUtil.setCellStyleProperty(sheet.getRow(rowIndx).getCell(j), workbook, CellUtil.DATA_FORMAT, HSSFDataFormat.getBuiltinFormat("0.00%"));
                }
            } else if(hrow3.getCell(j).getStringCellValue().trim().equals("%")){
                int successApp = totalSuccessAPPEachOfProductByTSR.get(hrow2.getCell(j-2).getStringCellValue().trim());
                sheet.getRow(rowIndx).createCell(j).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
                sheet.getRow(rowIndx).getCell(j).setCellFormula(String.format("IF(%s%d > 0,%s%d/(%d+%s%d), 0)",CellReference.convertNumToColString(j-2),rowIndx+1,CellReference.convertNumToColString(j-2),rowIndx+1,successApp,CellReference.convertNumToColString(j-2),rowIndx+1));
                sheet.getRow(rowIndx).getCell(j).setCellStyle(headCellStyle2);
                CellUtil.setCellStyleProperty(sheet.getRow(rowIndx).getCell(j), workbook, CellUtil.DATA_FORMAT, HSSFDataFormat.getBuiltinFormat("0.00%"));
            } else {
                sheet.getRow(rowIndx).createCell(j).setCellType(HSSFCell.CELL_TYPE_NUMERIC);
    ...
    public String getRowEachOfTotal() {
        return rowEachOfTotal;
    }

    public void setRowEachOfTotal(String rowEachOfTotal) {
        this.rowEachOfTotal = rowEachOfTotal;
    }
}
