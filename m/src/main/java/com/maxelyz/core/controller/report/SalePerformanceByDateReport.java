...
@ManagedBean
@RequestScoped
public class SalePerformanceByDateReport extends AbstractReportBaseController {
    private static Logger log = Logger.getLogger(SalePerformanceByDateReport.class);
    @PostConstruct
    public void initialize() {
        if (!SecurityUtil.isPermitted("report:saleperformancebydate:view")) {
    ...
    public String getReportPath() {
        return "/report/salePerformanceByDate.jasper";
//        return "/report/custom/lmg/salePerformanceByDate.jasper";
    }
    ...
    public String getQuery() {
        ...
        String select = "select convert(date, rpt.sale_date) as sale_date,"
...
//              + "sum(list_used)- sum(ISNULL(list_new,0)) as list_old, "
                + "sum(ISNULL(list_old,0)) as list_old, "
                ...
                + "and cast(po1.purchase_date as date) = cast(rpt.sale_date as date) "
...
//                + "sum(total_old_contact) as total_old_contact, "
       ...
       /* if (productId!=0)
        ...
        String groupBy = "GROUP BY rpt.sale_date ";
        String orderBy = "ORDER BY rpt.sale_date ";
