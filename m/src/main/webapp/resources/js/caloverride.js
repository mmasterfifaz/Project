...
(function ($, rf) {
    ...
    var CalendarView = {
        ...
        getSelectedDateControl: function(calendar) {
            ...
            // ldk begin set selected date
            try{
                if ( calendar.options.thailocale && calendar.selectedDate!= null){
                    var transformDate = new Date( calendar.selectedDate.getTime() );
                    transformDate.setFullYear( transformDate.getFullYear()+543 );
                    text = rf.calendarUtils.formatDate(transformDate, (calendar.timeType ? calendar.datePattern : calendar.options.datePattern), calendar.options.monthLabels, calendar.options.monthLabelsShort);
                }
            }catch(e){}
            // ldk end
        ...
        currentMonthControl: function (context) {
            // ldk begin show current month year
            ...
            if (context.calendar.options.thailocale){
                var transformDate = new Date( context.calendar.getCurrentDate().getTime() );
                transformDate.setFullYear( transformDate.getFullYear()+543 );
                text = rf.calendarUtils.formatDate(transformDate, "MMMM, yyyy", context.monthLabels, context.monthLabelsShort);
            }
            // ldk end
    ...
    rf.ui.Calendar = function(componentId, locale, options, markups) {
        ...
        // ldk begin : add locale in option
        var thailocal = (locale!=null && locale.indexOf("TH")>-1);
        this.options['thailocale'] = thailocal;
        // ldk end 
        // ldk begin (set back to ad year)
        if (this.options['thailocale'] && this.options.currentDate!=null){
            this.options.currentDate.setFullYear( this.options.currentDate.getFullYear()-543);
        }
        // ldk end 
    ...
    $.extend(rf.ui.Calendar.prototype, {
            ...
            scrollEditorYear: function(value) {
                ...
                if (element) {
                    ...
                    for (var i = 0; i < 5; i++) {
                        ...
                        // ldk begin : show year list
                        var yearPresent = (this.options.thailocale)? year+543:year ;
                        // ldk end
                        div.firstChild.innerHTML = yearPresent ;
                        if (year == this.dateEditorYear) {
                            $(div.firstChild).addClass('rf-cal-edtr-btn-sel');
                            this.dateEditorYearID = div.firstChild.id;
                        }
                        div = div.nextSibling;
                        div.firstChild.innerHTML = yearPresent + 5;
            ...
            createDECell: function(id, value, buttonType, param, className) {
                if (buttonType == 0) {
                    ...
                    value = (buttonType==2 && this.options.thailocale )?value+543:value;
            ...
            showPopup: function(e) {
                ...
                if (this.invokeEvent("show", element, e)) {
                    ...
                    if (baseInput.value) {
                        //ldk begin: set back to calendar
                        var text = baseInput.value;
                        try{
                            if (this.options.thailocale){
                                var d1 = rf.calendarUtils.parseDate(text, this.options.datePattern, this.options.monthLabels, this.options.monthLabelsShort);
                                d1.setFullYear(d1.getFullYear()-543);
                                text = rf.calendarUtils.formatDate(d1, this.options.datePattern, this.options.monthLabels, this.options.monthLabelsShort);
                            }
                        }catch(e){
                        }
                        this.__selectDate(text, false, {event:e, element:element});
            ...
            __getSelectedDateString: function(pattern) {
                ...
                //ldk begin return date 
                console.log(this.selectedDate);
                var text = rf.calendarUtils.formatDate(this.selectedDate, pattern, this.options.monthLabels, this.options.monthLabelsShort);
                try{
                    if (this.options.thailocale){
                        var transformDate = new Date(this.selectedDate.getTime());
                        transformDate.setFullYear(this.selectedDate.getFullYear()+543);
                        text = rf.calendarUtils.formatDate(transformDate, pattern, this.options.monthLabels, this.options.monthLabelsShort);
                    }
                }catch(ex){ }
                return text;
            },
